...
class YFinanceTools(Toolkit):
    def __init__(
        ...
        if stock_price or enable_all:
            self.register(self.get_current_stock_price, name="yfinance_get_current_stock_price")
        if company_info or enable_all:
            self.register(self.get_company_info, name="yfinance_get_company_info")
        if stock_fundamentals or enable_all:
            self.register(self.get_stock_fundamentals, name="yfinance_get_stock_fundamentals")
        if income_statements or enable_all:
            self.register(self.get_income_statements, name="yfinance_get_income_statements")
        if key_financial_ratios or enable_all:
            self.register(self.get_key_financial_ratios, name="yfinance_get_key_financial_ratios")
        if analyst_recommendations or enable_all:
            self.register(self.get_analyst_recommendations, name="yfinance_get_analyst_recommendations")
        if company_news or enable_all:
            self.register(self.get_company_news, name="yfinance_get_company_news")
        if technical_indicators or enable_all:
            self.register(self.get_technical_indicators, name="yfinance_get_technical_indicators")
        if historical_prices or enable_all:
            self.register(self.get_historical_stock_prices, name="yfinance_get_historical_stock_prices")
    ...
    def get_key_financial_ratios(self, symbol: str) -> str:
        ...
        try:
            ...
            logger.error(f"Error fetching key financial ratios for {symbol}: {e}")
